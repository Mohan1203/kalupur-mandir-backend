...
<div class="container-fluid px-4 py-3">
    <!-- Add Main Photo Gallery Form -->
    <div class="card shadow-sm border-0 mb-4">
        <div class="card-header bg-white py-3 border-bottom">
            <h5 class="mb-0 fw-bold text-dark">
                <i class="fas fa-images text-primary me-2"></i>
                Add Main Photo Gallery Image
            </h5>
        </div>
        <div class="card-body">
    ...
    <!-- Add Sub Photo Gallery Form -->
    <div class="card shadow-sm border-0 mb-4">
        <div class="card-header bg-white py-3 border-bottom">
            <h5 class="mb-0 fw-bold text-dark">
                <i class="fas fa-layer-group text-primary me-2"></i>
                Add Sub Photo Gallery Image
            </h5>
        </div>
        <div class="card-body">
        ...
        <div class="card-header bg-white py-3 border-bottom">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
                <div>
                    ...
                    <small class="text-muted">Total {{ count($images) }} main image(s)</small>
                </div>
                            ...
                            <tr class="border-bottom">
                                ...
                                <td class="px-3 py-3">
                                    <div class="d-flex align-items-center">
                                        <div class="flex-grow-1">
                                            <div class="fw-semibold text-dark mb-1">{{ $image['title'] }}</div>
                                ...
                                <td class="px-3 py-3 text-center">
                                    <div class="image-container">
                                        <img src="{{ asset(env('APP_URL').  '/' . $image['image']) }}" 
                                             class="rounded border shadow-sm" 
                                             width="60" height="60" 
                                             alt="Photo Image"
                                             style="object-fit: cover; border: 2px solid #dee2e6; cursor: pointer;"
                                             onclick="showImageModal('{{ asset(env('APP_URL').  '/' . $image['image']) }}', '{{ $image['title'] }}')">
                                    </div>
                                </td>
                                <td class="px-3 py-3 text-center">
                                    @php
                                        $subCount = $subImages->where('photo_id', $image['id'])->count();
                                    @endphp
                                    <span class="badge bg-info text-white">{{ $subCount }} sub image(s)</span>
                                </td>
                                <td class="px-3 py-3 text-center">
                                    <div class="d-flex gap-1 justify-content-center">
                                        <a href="/editphotogallery/{{ $image['id'] }}" 
                                           class="btn btn-sm btn-outline-primary" 
                                           title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form action="{{ route('handle.deletephotogallery', $image['id']) }}" 
                                              method="post" 
        ...
        <div class="card-header bg-white py-3 border-bottom">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
                <div>
                    ...
                    <small class="text-muted">Total {{ count($subImages) }} sub image(s)</small>
                </div>
                <div class="d-flex gap-2">
                    <div class="input-group" style="width: 300px;">
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0" id="subImagesTable">
                    ...
                    <tbody>
                        @forelse ($subImages as $key => $subimage)
                            <tr class="border-bottom">
                                <td class="px-3 py-3 text-center">
                                    <span class="badge text-white rounded-pill" style="background-color: #5d1a1e;">{{ $key + 1 }}</span>
                                </td>
                                <td class="px-3 py-3">
                                    @php
                                        $parentImage = $images->where('id', $subimage['photo_id'])->first();
                                    @endphp
                                    <div class="d-flex align-items-center">
                                        <div class="flex-grow-1">
                                            <div class="fw-semibold text-dark mb-1">{{ $parentImage ? $parentImage['title'] : 'Unknown' }}</div>
                                            <small class="text-muted">ID: {{ $subimage['photo_id'] }}</small>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-3 py-3">
                                    <div class="fw-semibold text-dark">{{ $subimage['title'] }}</div>
                                    <small class="text-muted">Sub Gallery Image</small>
                                </td>
                                <td class="px-3 py-3 text-center">
                                    <div class="image-container">
                                        <img src="{{ asset(env('APP_URL').  '/' . $subimage['image']) }}" 
                                             class="rounded border shadow-sm" 
                                             width="60" height="60" 
                                             alt="Sub Photo Image"
                                             style="object-fit: cover; border: 2px solid #dee2e6; cursor: pointer;"
                                             onclick="showImageModal('{{ asset(env('APP_URL').  '/' . $subimage['image']) }}', '{{ $subimage['title'] }}')">
                                    </div>
                                </td>
                                <td class="px-3 py-3 text-center">
                                    <div class="d-flex gap-1 justify-content-center">
                                        <a href="/editsubphotogallery/{{ $subimage['id'] }}" 
                                           class="btn btn-sm btn-outline-primary" 
                                           title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form action="{{ route('handle.deletesubphotogallery', $subimage['id']) }}" 
                                              method="post" 
                                              style="display: inline;">
                                            @csrf
                                            @method('delete')
